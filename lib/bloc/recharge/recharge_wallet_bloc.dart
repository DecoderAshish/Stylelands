...
class RechargeWalletBloc
  ...
  void onContinueTap() async {
    ...
    if (settings?.paymentGateway == 1) {
      ...
      //   },
      // );
    } else if (settings != null && settings?.paymentGateway == 7) {
      if (settings?.sslcommerzStoreId == null ||
          settings!.sslcommerzStoreId!.isEmpty) {
        AppRes.showSnackBar('Store id Missing', false);
        return;
      } else if (settings!.sslcommerzStorePassword == null ||
          settings!.sslcommerzStorePassword!.isEmpty) {
        AppRes.showSnackBar('Store Password Missing', false);
        return;
      } else if (settings!.sslcommerzCurrencyCode == null ||
          settings!.sslcommerzCurrencyCode!.isEmpty) {
        AppRes.showSnackBar(
            'This Payment Gateway currency code missing', false);
        return;
      ...
      SSLCommerz().makePayment(
        totalAmount: selectedAmount.toString(),
        storeId: settings?.sslcommerzStoreId ?? '',
        storePassword: settings?.sslcommerzStorePassword ?? '',
        currency: settings?.sslcommerzCurrencyCode ?? 'USD',
        // onError: (error) {
        //   AppRes.showSnackBar(error, false);
        // },
