...
class AppRes {
  ...
  static List<SlotData> sortingSlots(
    List<SlotData>? slots,
    int month,
    int day,
    bool isAvailable,
    ...
    List<SlotData> sortedSlots = isAvailable
        ? slots?.where(
              (element) {
                DateTime dateTime = DateTime(
                    DateTime.now().year,
                    month,
                    day,
                    int.parse(element.time?.substring(0, 2) ?? '0'),
                    int.parse(element.time?.substring(2, 4) ?? '0'));

                return element.available == true &&
                    dateTime.difference(current).inMinutes >= 5;
              },
            ).toList() ??
            []
        : slots?.where(
